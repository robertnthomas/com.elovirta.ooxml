...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:x="com.elovirta.ooxml" exclude-result-prefixes="x xs opentopic" version="2.0">

  <xsl:import href="document.utils.xsl"/>
  <xsl:import href="flatten.xsl"/>

  <xsl:param name="topic.numbers" as="xs:boolean" select="true()"/>
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="lists" as="xs:string*">
      <xsl:for-each
        select="$content//*[contains(@class, ' topic/ol ') or contains(@class, ' topic/ul ') or contains(@class, ' topic/sl ')]">
  ...
  <xsl:template match="@xtrc | @xtrf | *[contains(@class, '- topic/required-cleanup ')]"
    mode="number" priority="1000"/>
  ...
  <xsl:template match="*[contains(@class, ' topic/draft-comment ')]" mode="number">
    ...
    <xsl:copy>
      <xsl:attribute name="x:draft-comment-number"
        select="index-of($draft-comment-lists, generate-id(.)) + 100"/>
      <xsl:apply-templates select="@* | node()" mode="number"/>
    </xsl:copy>
  </xsl:template>
  ...
  <xsl:template match="*[contains(@class, ' topic/xref ')][@scope = 'external']" mode="number">
    ...
    <xsl:copy>
      <xsl:attribute name="x:external-link-number"
        select="index-of($external-link-lists, generate-id(.)) + 100"/>
      <xsl:apply-templates select="@* | node()" mode="number"/>
    </xsl:copy>
  </xsl:template>
  ...
  <xsl:template match="*[contains(@class, ' topic/topic ')]" mode="number">
    <xsl:copy>
      <xsl:if test="$topic.numbers">
        <xsl:for-each select="key('map-id', @id)[1]">
          <xsl:if
            test="
              empty(ancestor-or-self::*[self::*[contains(@class, ' bookmap/frontmatter ') or
              contains(@class, ' bookmap/backmatter ')] or
              @props = 'nonumbering'])">
            <xsl:attribute name="x:header-number">
              <xsl:variable name="appendix"
                select="ancestor-or-self::*[contains(@class, ' bookmap/appendix ')][1]"/>
              <xsl:choose>
                <xsl:when test="$appendix">
                  <xsl:for-each select="$appendix">
                    <xsl:number count="*[contains(@class, ' bookmap/appendix ')]" level="single"
                      format="A."/>
                  </xsl:for-each>
                  <xsl:number
                    count="
